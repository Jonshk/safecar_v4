...
class _TowScreenState extends State<TowScreen> {
  ...
  Future<void> _loadSavedRefs() async {
    ...
    setState(() {
      _savedRefs = prefs.getStringList('tow_references') ?? [];
    });
  }
  ...
  Future<void> _saveRef(String ref) async {
    ...
    if (!refs.contains(ref)) {
      refs.insert(0, ref);
      if (refs.length > 5) refs.removeLast(); // máximo 5 referencias guardadas
    }
  ...
  Future<void> _getLocation() async {
    ...
    try {
      ...
      if (!serviceEnabled) {
        setState(() { _locLoading = false; _locStatus = 'error'; });
      ...
      if (permission == LocationPermission.deniedForever) {
        setState(() { _locLoading = false; _locStatus = 'error'; });
        _showLocError('Location permission permanently denied. Enable it in Settings.');
      ...
      if (_pickup.text.trim().isEmpty) {
        _pickup.text = 'GPS: ${pos.latitude.toStringAsFixed(6)}, ${pos.longitude.toStringAsFixed(6)}';
      }
    } catch (e) {
      setState(() { _locLoading = false; _locStatus = 'error'; });
  ...
  Future<void> _submit() async {
    ...
    if (result != null) {
      ...
      await _saveRef(ref); // ← guardar en SharedPreferences
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      Container(
                        width: 56, height: 56,
                        decoration: BoxDecoration(
                          color: AppTheme.redGlow, borderRadius: BorderRadius.circular(16),
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text('TOW SERVICE', style: AppTheme.display(22, w: FontWeight.w900)),
                        const SizedBox(height: 4),
                        Text('24/7 Emergency Towing', style: AppTheme.body(13, color: AppTheme.white60)),
                      ])),
                    ]),
                  ).animate().fadeIn(duration: 400.ms).slideY(begin: -0.1, end: 0),
                ),

                // Solicitudes anteriores guardadas
                if (_savedRefs.isNotEmpty) SliverToBoxAdapter(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(16, 0, 16, 0),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('MY REQUESTS', style: AppTheme.mono(11, w: FontWeight.w700, color: AppTheme.white30)),
                      const SizedBox(height: 10),
                      ..._savedRefs.map((ref) => GestureDetector(
                        onTap: () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => TrackScreen(initialReference: ref),
                        )),
                        child: Container(
                          ...
                          child: Row(children: [
                            ...
                            Text('Track →', style: AppTheme.mono(11, color: AppTheme.white30)),
                          ]),
                        ),
                      )),
                ...
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: _LocationButton(
                      status: _locStatus, loading: _locLoading,
                      lat: _lat, lng: _lng, onTap: _getLocation,
                    ),
                  ...
                  child: Padding(
                    ...
                    child: Form(
                      ...
                      child: Column(children: [
                        ...
                        _Field(ctrl: _name, label: 'Full name', icon: Icons.person_outline_rounded,
                            validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null),
                        const SizedBox(height: 12),
                        _Field(ctrl: _phone, label: 'Phone number', icon: Icons.phone_outlined,
                            keyboard: TextInputType.phone,
                            validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null),
                        const SizedBox(height: 20),
                        _SectionLabel('VEHICLE & LOCATION'),
                        const SizedBox(height: 12),
                        _Field(ctrl: _vehicle, label: 'Vehicle (Year, Make, Model)', icon: Icons.directions_car_outlined,
                            validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null),
                        const SizedBox(height: 12),
                        _Field(ctrl: _pickup, label: 'Pickup address / location', icon: Icons.location_on_outlined,
                            validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null),
                        const SizedBox(height: 12),
                        _Field(ctrl: _destination, label: 'Destination (optional)', icon: Icons.flag_outlined),
                        const SizedBox(height: 12),
                        _Field(ctrl: _notes, label: 'Additional notes (optional)', icon: Icons.notes_rounded, maxLines: 3),
                          ...
                          child: AnimatedContainer(
                            ...
                            child: Center(
                              child: _loading
                                  ...
                                  : Text('REQUEST TOW', style: AppTheme.mono(14, w: FontWeight.w800)),
                            ),
                          ),
                        ),
...
class _LocationButton extends StatelessWidget {
  ...
  const _LocationButton({required this.status, required this.loading, required this.lat, required this.lng, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isOk ? const Color(0xFF00C47A) : isError ? AppTheme.red.withOpacity(0.5) : AppTheme.border,
            width: isOk ? 1.5 : 1,
          ),
        ),
        child: Row(children: [
          Container(
            ...
            child: loading
                ...
                : Icon(
                    isOk ? Icons.location_on_rounded : isError ? Icons.location_off_rounded : Icons.my_location_rounded,
                    color: isOk ? const Color(0xFF00C47A) : isError ? AppTheme.red : AppTheme.white60,
                    size: 22,
                  ),
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
              isOk ? 'Location captured' : isError ? 'Location failed' : 'Share my location',
              style: AppTheme.mono(13, w: FontWeight.w700, color: isOk ? const Color(0xFF00C47A) : isError ? AppTheme.red : AppTheme.white80),
            ),
            const SizedBox(height: 3),
            Text(
              isOk ? '${lat!.toStringAsFixed(5)}, ${lng!.toStringAsFixed(5)}' : isError ? 'Tap to try again' : 'Tap to send your exact GPS position',
              style: AppTheme.mono(11, color: isOk ? const Color(0xFF00C47A).withOpacity(0.7) : AppTheme.white30),
            ),
          ])),
          if (!loading) Icon(isOk ? Icons.check_circle_rounded : Icons.chevron_right_rounded, color: isOk ? const Color(0xFF00C47A) : AppTheme.white30, size: isOk ? 22 : 18),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text('TOW REQUESTED', style: AppTheme.display(26, w: FontWeight.w900)).animate(delay: 200.ms).fadeIn().slideY(begin: 0.2, end: 0),
        const SizedBox(height: 8),
        Text('A dispatcher will call you shortly.', style: AppTheme.body(14, color: AppTheme.white60), textAlign: TextAlign.center).animate(delay: 300.ms).fadeIn(),
        const SizedBox(height: 28),
        Container(
          ...
          child: Column(children: [
            Text('REFERENCE', style: AppTheme.mono(11, color: AppTheme.white30)),
            const SizedBox(height: 8),
            Text(reference, style: AppTheme.display(22, w: FontWeight.w800, color: AppTheme.red)),
            const SizedBox(height: 4),
            Text('Saved automatically — see "My Requests"', style: AppTheme.mono(11, color: AppTheme.white30)),
          ]),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Center(child: Text('TRACK LIVE', style: AppTheme.mono(13, w: FontWeight.w700, color: AppTheme.red))),
          ),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Center(child: Text('New Request', style: AppTheme.mono(13, w: FontWeight.w600))),
          ),
